...
from .models import League, Match, Player, PlayerStats, Team, TeamStanding, Transfer
    ...
    def create(self, validated_data):
        user = User.objects.create_user(
            ...
            email=validated_data.get(
                "email", ""
            ),  # Safely handles missing/empty email payload
...
class PlayerMatchStatsSerializer(serializers.ModelSerializer):
    ...
    home_score = serializers.IntegerField(
        source="match.home_score", read_only=True
    )
    away_score = serializers.IntegerField(
        source="match.away_score", read_only=True
    )
    date = serializers.DateField(source="match.date", read_only=True)
    time = serializers.TimeField(source="match.time", read_only=True)
    completed = serializers.BooleanField(
        source="match.completed", read_only=True
    )
